...
class ColposcopyController extends \BaseController {
    ...
    {
        return View::make("report.colposcopy");
    }
    ...
    public function makeTable(){
        ...
        }elseif(Input::get("show") == "Colposcopy Status"){
            $columnhistory = array('yes'=>'Colposcopy Done','no'=>'Colposcopy Not Done');
        }
            ...
            foreach($row as $key => $value){
                ...
                if(isset($columntype)){
                    foreach($columntype as $key1=>$value1){
                        ...
                        if($pat){
                            $que = $query[0]->whereIn('id', ColposcopyStatus::where('result_id',$key1)->get()->lists('patient_id')+array('0'))->whereBetween('created_at',array($from,$to));
                            $column[$value1][] = $que->count();
                        }elseif($vis){
                            $que = $query[1]->whereIn('id', ColposcopyStatus::where('result_id',$key1)->get()->lists('visit_id')+array('0'))->whereBetween('created_at',array($from,$to));
                            $column[$value1][] = $que->count();
                        }
                    }
                    ...
                    foreach($columnhistory as $key1=>$value1){
                        ...
                        $query = $this->processQuery($patientquery,$visitquery);
                        if($pat){
                            $que = $query[0]->whereIn('id', ColposcopyStatus::where('status',$key1)->get()->lists('patient_id')+array('0'))->whereBetween('created_at',array($from,$to));
                            $column[$value1][] = $que->count();
                        }elseif($vis){
                            $que = $query[1]->whereIn('id', ColposcopyStatus::where('status',$key1)->get()->lists('visit_id')+array('0'))->whereBetween('created_at',array($from,$to));
                            $column[$value1][] = $que->count();
                        }
                    }
                    $title = Input::get('vertical')." ". $query[2]." ".Input::get('year');
            ...
            for($i=$range;$i<=$limit;$i+=$range){
                ...
                if(isset($columntype)){
                    foreach($columntype as $key1=>$value1){
                        ...
                        $visitquery   = DB::table('visit');
                        $query = $this->processQuery($patientquery,$visitquery);
                        $que = $query[0]->whereIn('id', ColposcopyStatus::where('result_id',$key1)->get()->lists('patient_id')+array('0'))->whereBetween('birth_date',array($end,$start));
                        $column[$value1][] = $que->count();
                    }
                    ...
                    foreach($columnhistory as $key1=>$value1){
                        ...
                        $visitquery   = DB::table('visit');
                        $query = $this->processQuery($patientquery,$visitquery);
                        $que = $query[0]->whereIn('id', ColposcopyStatus::where('status',$key1)->get()->lists('patient_id')+array('0'))->whereBetween('birth_date',array($end,$start));
                        $column[$value1][] = $que->count();
                    }
                    $title = Input::get('vertical')." Age Range ". $query[2];
                }
