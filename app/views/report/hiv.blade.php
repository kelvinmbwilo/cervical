...
<div class='form-group'>
    <div class='col-sm-2'>
        Show<br>{{ Form::select('show',array("HIV Status"=>"HIV Status","CD4 Count"=>"CD4 Count","Test  Results"=>"Test Results","Decline Reason"=>"Decline Reason"),'',array('class'=>'form-control','required'=>'requiered')) }}
    </div>
    <div class='col-sm-2'>
        <span class="all">
        ...
        <span class="other">
            Vertical(Y-axis)<br>{{ Form::select('vertical',array("Visits"=>"Visits"),'',array('class'=>'form-control','required'=>'requiered')) }}
        </span>
    </div>
...
<script>
    $(document).ready(function (){
        ...
        $("select[name=show]").change(function(){
            if($(this).val() == "Test  Results" || $(this).val() == "Decline Reason" ){
                $(".other").html(other);
                $(".all").html("");
            }else{
                $(".other").html("");
        ...
        $("#table").unbind("click").click(function(){
            ...
            $("#formms").ajaxSubmit({
                url:"<?php echo url('report/hiv/table') ?>",
                target: '#chartarea',
                data: {chat:'table'},
        ...
        $("#pie").unbind("click").click(function(){
            ...
            $("#chartarea").html("<h3><i class='fa fa-spin fa-spinner '></i><span>Loading...</span><h3>");
            $("#formms").ajaxSubmit({
                url:"<?php echo url('report/hiv/pie') ?>",
                target: '#chartarea',
                data: {chat:'table'},
                success:  afterSuccess
            });
        ...
        $("#bar").unbind("click").click(function(){
            ...
            $("#chartarea").html("<h3><i class='fa fa-spin fa-spinner '></i><span>Loading...</span><h3>");
            $("#formms").ajaxSubmit({
                url:"<?php echo url('report/hiv/bar') ?>",
                target: '#chartarea',
                data: {chat:'table'},
                success:  afterSuccess
            });
        ...
        $("#line").unbind("click").click(function(){
            ...
            $("#chartarea").html("<h3><i class='fa fa-spin fa-spinner '></i><span>Loading...</span><h3>");
            $("#formms").ajaxSubmit({
                url:"<?php echo url('report/hiv/line') ?>",
                target: '#chartarea',
                data: {chat:'table'},
                success:  afterSuccess
            });
